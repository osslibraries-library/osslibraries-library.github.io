...
`osslibraries_ui` 包提供两个可嵌入应用的 ArkUI 页面：依赖列表页，以及每个依赖对应的详情页。
...
页面通过命名路由声明，无需修改 `main_pages.json`。在 `entry` 模块的页面文件顶部导入它们以注册路由：
...
列表页通过 `LibsLoader` 自行加载数据，无需传入参数。每行渲染一个库，点击后打开详情页。

## 详情页

列表页会把库的 `uniqueId` 交给详情页：
...
详情页读取参数，通过 `LibsHolder` 查找库，并展示其链接与完整许可证全文。库既无许可证也无网站时，对应行不可点击。
