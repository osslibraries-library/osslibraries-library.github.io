...
OSSLibraries 为 HarmonyOS 应用提供开源许可证的展示能力。它在构建时扫描 OHPM 依赖，将许可证信息写入应用的资源目录；运行时将其渲染为包含列表与详情视图的自适应页面。
...
- **`osslibraries`**（HarmonyOS 库）——运行时的部分。负责从 rawfile 中读取、解析数据，提供两种用法：使用预定义的自适应页面，或仅使用其核心 API 自行构建界面。
